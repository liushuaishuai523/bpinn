...
parser.add_argument('--filename', type=str, default='4-FI-BPINN_0dot0264_0dot1_10_10_10', help='保存数据文件名')
...
parser.add_argument('--w_bc', type=float, default=10.0, help='边界损失权重')
parser.add_argument('--w_ini', type=float, default=10.0, help='初始条件损失权重')
parser.add_argument('--w_g', type=float, default=10.0, help='耦合项损失权重')
...
def train(model_1):
    ...
    for epoch in range(args.e):
        def closure():
            ...
            ext_bc = bd_fun(x_b_1)
            bc_loss_1 = torch.mean((bc_pred - ext_bc) ** 2)  # bd_fun always returns 1
            ...
            # 组合损失
            loss = args.w_pde * pde_loss_1 + args.w_bc * bc_loss_1 + args.w_g * loss_g + args.w_ini * loss_ini + args.w_time * loss_time
