...
parser.add_argument('--ranges', type=list, default=[[-1, 1], [0, 0.0264]], help='空间，时间范围')
...
parser.add_argument('--filename', type=str, default='4-A-BPINN_0dot0264_0dot1_10_10_10', help='保存数据文件名')
...
parser.add_argument('--w_ini', type=float, default=10, help='初始条件损失权重')
parser.add_argument('--w_g', type=float, default=10, help='耦合项损失权重')
...
initial_fun = lambda x: 1/(2*(torch.cos(torch.pi * x[:, 0:1] / 2) + 1)**2)
...
def Operator(U, gU, X, domain):
    ...
    if domain == 'domain_1':
        output = delta + U_xx / (U+1)**3 + gU_t  # 格式2
    else:
...
def generate_solution_on_grid(model, x_test, grid_shape):
    ...
    u_pred = -1+torch.sqrt(1/(torch.abs(2*g_pred)))
...
def train(model_1):
    ...
    for epoch in range(args.e):
        def closure():
            ...
            bc_lower_loss = torch.mean((bc_lower_pred - 1/2) ** 2)  # bd_fun always returns 1

            bc_upper_pred = model_1(x_b_upper)[:, 1:2]
            bc_upper_loss = torch.mean((bc_upper_pred - 1/2) ** 2)  # bd_fun always returns 1
            ...
            # 耦合项损失
            loss_g = torch.mean((gu_1 - (1 / 2 * 1 / (u_1 + 1) ** 2)) ** 2)
