...
parser.add_argument('--filename', type=str, default='FI-BPINN_0dot9598_0dot1_8_5_3', help='保存数据文件名')
...
def plot_3d_surface(solution, x_vals, y_vals, name='3d_solution.png', title='3D Solution Surface'):
    folder = f'./results/{filename}'
    os.makedirs(folder, exist_ok=True)
    ...
    # 调整轴比例（重点修改）
    ax.set_box_aspect([np.ptp(X), 3 * np.ptp(Y), np.ptp(solution)])  # 将 Y 轴比例放大 3 倍

    # 其余设置保持不变...
    cbar = fig.colorbar(surf, ax=ax, shrink=0.6, aspect=20, pad=0.1)
    cbar.set_label('u(x,t)', rotation=270, labelpad=20)
    ...
    # 保存图像
    plt.savefig(os.path.join(folder, name), dpi=600, bbox_inches='tight', facecolor='white')
    plt.close()
...
def train(model_1):
    ...
    for epoch in range(args.e):
        ...
        if epoch % 10 == 0:
            ...
            # 绘制3D图
            plot_3d_surface(solution_on_grid, x_vals, y_vals, '3d_solution.png', '3D Model Solution')
            plot_3d_surface(ref_solution, x_vals, y_vals, '3d_reference.png', '3D Reference Solution')
            plot_3d_surface(abs_error, x_vals, y_vals, '3d_absolute_error.png', '3D Absolute Error')
