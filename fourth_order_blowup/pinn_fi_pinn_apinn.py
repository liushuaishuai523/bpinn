...
parser.add_argument('--ranges', type=list, default=[[0, 5], [0, 0.925]], help='空间，时间范围')
...
parser.add_argument('--filename', type=str, default='4-FI-PINN_0dot925_0dot1_8_5_3', help='保存数据文件名')
...
# 添加FI-PINNs参数
parser.add_argument('--use_fi', type=int, default=1, choices=[0, 1], help='是否启用FI-PINNs自适应采样')
...
def Operator(U, gU, X, domain):
    ...
    if domain == 'domain_1':
        output =  U_t + U_xxxx-delta*torch.exp(U)
    else:
...
def train(model_1):
    ...
    for epoch in range(args.e):
        def closure():
            ...
            # 计算二阶导数边界条件
            bc_lower_pred_x = grad(bc_lower_pred, x_b_lower)[:, 0:1]  # 修正：使用 x_b_lower
            bc_lower_pred_xx = grad(bc_lower_pred_x, x_b_lower)[:, 0:1]  # 修正：使用 x_b_lower

            bc_upper_pred_x = grad(bc_upper_pred, x_b_upper)[:, 0:1]  # 修正：使用 x_b_upper
