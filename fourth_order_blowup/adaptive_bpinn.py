...
parser.add_argument('--ranges', type=list, default=[[0, 5], [0, 0.95]], help='空间，时间范围')
...
parser.add_argument('--filename', type=str, default='0-A-BPINN_0dot95_0dot1_8_5_3', help='保存数据文件名')
...
parser.add_argument('--w_g', type=float, default=3, help='耦合项损失权重')
...
# 添加APINN自适应权重参数
parser.add_argument('--use_apinn', type=int, default=1, choices=[0, 1],
                    help='是否启用APINN自适应权重算法 (0:禁用, 1:启用)')
parser.add_argument('--apinn_alpha', type=float, default=5,
                    help='APINN权重调整系数α')
...
def train(model_1):
    ...
    for epoch in range(args.e):
        def closure():
            ...
            # 边界损失
            bc_lower_pred = model_1(x_b_lower)[:, 1:2]
            bc_lower_loss = torch.mean((bc_lower_pred - 1) ** 2)  # bd_fun always returns 1

            bc_upper_pred = model_1(x_b_upper)[:, 1:2]
            bc_upper_loss = torch.mean((bc_upper_pred - 1) ** 2)  # bd_fun always returns 1

            # 计算二阶导数边界条件
            u_lower_pred = model_1(x_b_lower)[:, 0:1]
            u_upper_pred = model_1(x_b_upper)[:, 0:1]
            bc_lower_pred_x = grad(u_lower_pred, x_b_lower)[:, 0:1]  # 修正：使用 x_b_lower
            bc_lower_pred_xx = grad(bc_lower_pred_x, x_b_lower)[:, 0:1]  # 修正：使用 x_b_lower

            bc_upper_pred_x = grad(u_upper_pred, x_b_upper)[:, 0:1]  # 修正：使用 x_b_upper
            bc_upper_pred_xx = grad(bc_upper_pred_x, x_b_upper)[:, 0:1]  # 修正：使用 x_b_upper

            bc_lower_loss_xx = torch.mean((bc_lower_pred_xx - 0) ** 2)
            bc_upper_loss_xx = torch.mean((bc_upper_pred_xx - 0) ** 2)
            ...
            if args.use_apinn:
                loss = (lambda_weights['ini'] * loss_ini +
                        ...
                        lambda_weights['g'] * loss_g +
                        lambda_weights['bc_lower'] * bc_lower_loss_xx +
                        lambda_weights['bc_upper'] * bc_upper_loss_xx +
                ...
                loss = (args.w_ini * loss_ini +
                        ...
                        args.w_g * loss_g +
                        args.w_bc * (bc_lower_loss_xx + bc_upper_loss_xx) +
        ...
        if args.use_apinn and epoch % args.apinn_interval == 0 and epoch > 0:
            ...
            if len(avg_losses) == 5:  # 确保所有损失项都有数据
