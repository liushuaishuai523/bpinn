...
parser.add_argument('--number', type=list, default=[400, 100], help='内点，边界取点数')
parser.add_argument('--ranges', type=list, default=[[-1, 1], [0, 0.16664]], help='空间，时间范围')
...
parser.add_argument('--filename', type=str, default='1-FI-BPINN_0dot16664_0dot1_10_5_2', help='保存数据文件名')
...
parser.add_argument('--w_g', type=float, default=2.0, help='耦合项损失权重')
...
def train(model_1):
    ...
    for epoch in range(args.e):
        ...
        if epoch % 10 == 0:
            ...
            plt.yscale('log')
            plt.grid(True, which="both", ls="--")
            plt.savefig(os.path.join(folder, f'final_time_error.pdf'), dpi=300,
                        bbox_inches='tight')
            plt.close()

            # 保存终止时刻的相对误差数据
            final_error_data = np.column_stack((x_vals, final_rel_error))
            np.savetxt(os.path.join(folder, f'final_time_error.txt'), final_error_data,
